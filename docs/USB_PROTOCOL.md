...
## Firmware default visuals (firmware 1.8.10)

PIXEL PRO 1.8.10 always has a factory Main Menu wallpaper and an animated
screensaver available even when the user has never uploaded media.

- The factory visual was redesigned from the user's blue/cyan/teal reference:
  bright aqua sky, translucent-looking turquoise layers, smooth cyan ribbons,
  and a deep navy foreground. The old pink/orange orbs and coarse polygon waves
  from 1.8.9 were removed.
- The fallback is rendered as full-width RGB565 scanlines with per-pixel
  ...
  commands.
- The Main Menu uses a static frame from the same visual. The existing eight
  profile/action icons and bottom status strip are still drawn on top.
- A valid user Main Menu JPEG remains higher priority. `MENUBGCLEAR|<profile>`
  removes only that user override; the new aqua factory wallpaper is shown
  immediately.
- A valid user GIF/JPEG/PXQ screensaver remains higher priority. If none is
  stored, or after `SAVCLEAR`, the factory aqua animation is active.
- The factory animation now runs at 20 FPS. Each layer drifts independently to
  create a slow, seamless motion without the old 12 FPS orb animation.
- `SAVERSTATE` reports `READY` for the factory screensaver.
- `SAVMEDIA` reports the factory fallback as
  `STATE=READY|KIND=DEFAULT` with a 480×320 logical size and 20 FPS.
