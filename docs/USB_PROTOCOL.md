...
Firmware 1.4.0 keeps native USB HID + CDC, 20 keymap profiles, live memory telemetry, Lumi Action bindings and the ILI9486 480×320 i8080 display. GIF files stay compressed and are decoded on-device; v1.3.5 also accepts arbitrary GIF canvas sizes up to 1024×1024 and scales them to the panel at render time.

## HELLO

HELLO / GET_INFO returns a line containing:

PIXELPRO|1|FW=1.4.0|MCU=ESP32S2|KEYS=8|PROFILES=20|LAYERS=4|MACROS=20|ACTIONS=32|DISPLAY=ILI9486,480x320,i8080-8|CAPS=HID,CDC,KEYMAP,LAYERS,HOST_MACRO,HOST_ACTION,MEM,PANEL,SAVER,MEDIA,DIRECT_GIF,DIRECT_JPEG,RGB_PER_KEY,ROM_BOOT|VID=303A|PID=80C2
...
Firmware 1.4.0 disables the Arduino USB CDC DTR/RTS reboot hook during normal
...
Firmware 1.4.0 keeps normal CDC reboot handling disabled. Lumi Macropad must first send:
