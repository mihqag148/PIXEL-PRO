...
## Firmware default visuals (firmware 1.8.9)

PIXEL PRO 1.8.9 always has a factory Main Menu wallpaper and an animated
screensaver available even when the user has never uploaded media.

- Both factory visuals are generated by firmware drawing primitives in the
  compiled application image. They consume no LittleFS media space and cannot
  be deleted through the USB media commands.
- The palette uses blue, cyan, purple, pink, and a warm orange accent, matching
  the macOS/iOS-inspired visual direction used by PIXEL PRO.
- The Main Menu factory wallpaper is only the background. The existing eight
  profile/action icons and the bottom dock/status UI are still drawn normally
  on top.
- A valid user Main Menu JPEG remains higher priority. `MENUBGCLEAR|<profile>`
  removes only that user override; the factory wallpaper is shown immediately.
- A valid user GIF/JPEG/PXQ screensaver remains higher priority. If none is
  stored, or after `SAVCLEAR`, the firmware default animation is active.
- The factory animation runs at 12 FPS to keep the fallback lightweight.
- `SAVERSTATE` reports `READY` for the factory screensaver.
- `SAVMEDIA` reports the factory fallback as
  `STATE=READY|KIND=DEFAULT` with a 480×320 logical size.
- Because the fallback is inside the firmware application rather than a
  LittleFS file, reformatting or clearing LittleFS cannot remove it.
