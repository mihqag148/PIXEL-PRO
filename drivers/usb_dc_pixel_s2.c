...
#define DT_DRV_COMPAT pixel_esp32s2_usb
/* ESP32-S2 adaptation of Zephyr 10ba6d0 USB DesignWare driver. */
...
static int pixel_usb_clock_enable(void)
{
    ...
    /* Bus-powered, fixed peripheral role. No external VBUS-sense pin. */
    ...
    esp_rom_gpio_connect_in_signal(GPIO_MATRIX_CONST_ZERO_INPUT, USB_SRP_SESSEND_IN_IDX, false);
    return 0;
}
...
#define USB_DW_DEVICE_DEFINE(n)							\
	...
	static const struct usb_dw_config usb_dw_cfg_##n = {			\
		...
		.clk_enable_func = pixel_usb_clock_enable,		\
...
static int usb_dw_init(void)
{
	...
	/* S2 has 1024 FIFO words. RX=256; EP0..5 each 64 words. */
    base->grxfsiz = 256;
    base->gnptxfsiz = (64U << 16) | 256U;
    for (int i = 0; i < 5; i++) {
        base->dieptxf[i] = (64U << 16) | (320U + i * 64U);
    }
    base->gusbcfg = (base->gusbcfg & ~(0xFU << 10)) | (5U << 10);
    base->gotgctl |= BIT(6) | BIT(7); /* B-session valid override. */
